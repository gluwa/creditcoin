...
mod pallet {
	...
	use frame_support::{
		pallet_prelude::*,
		traits::{
			Currency, DefensiveResult, DefensiveSaturating, EnsureOrigin, Get, LockIdentifier,
	...
	pub(super) const STAKING_ID: LockIdentifier = *b"staking ";
	...
	pub trait Config: frame_system::Config {
		...
		type OnStakerSlash: sp_staking::OnStakerSlash<Self::AccountId, Self::CurrencyBalance>;
	...
	/// The minimum active bond to become and maintain the role of a prover.
	#[pallet::storage]
	pub type MinStakerBond<T: Config> = StorageValue<_, BalanceOf<T>, ValueQuery>;
	...
	//#[pallet::storage] pub(crate) type SpanSlash<T: Config> = StorageMap< _, Twox64Concat, (T::AccountId, slashing::SpanIndex), slashing::SpanRecord<BalanceOf<T>>, ValueQuery, >;
	...
	pub struct GenesisConfig<T: Config> {
		pub min_staker_bond: BalanceOf<T>,
		// stash, controller, balance
		pub stakers: Vec<(T::AccountId, T::AccountId, T::CurrencyBalance)>,
	}

	#[cfg(feature = "std")]
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			GenesisConfig { min_staker_bond: Default::default(), stakers: Default::default() }
		}
	}
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			MinStakerBond::<T>::put(self.min_staker_bond);

			for (stash, controller, balance) in &self.stakers {
				logger!(trace, "inserting genesis staker: {:?} => {:?}", stash, balance);
				assert!(
	...
	pub enum Error<T> {
		...
		/// governance (see `MinStakerBond` and `MinNominatorBond`). If unbonding is the
		...
		TooManyNominators,
		/// There are too many stakers in the system. Governance needs to adjust the
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(2)]
		#[pallet::weight(
            T::WeightInfo::withdraw_unbonded_kill(32u32).saturating_add(T::WeightInfo::unbond()))
        ]
		pub fn unbond(
			origin: OriginFor<T>,
			#[pallet::compact] value: BalanceOf<T>,
			...
			if !value.is_zero() {
				...
				let min_active_bond = MinStakerBond::<T>::get();
					...
					ledger.unlocking.last_mut().filter(|chunk| chunk.era == era)
				{
					...
					ledger
						...
						.map_err(|_| Error::<T>::NoMoreChunksSlots)?;
				};
				// NOTE: ledger must be updated prior to calling `Self::weight_of`.
				Self::update_ledger(&controller, &ledger);
		...
		pub fn cancel_deferred_slash(
			...
			ensure!(super::is_sorted_and_unique(&slash_indices), Error::<T>::NotSortedAndUnique);
		...
		pub fn set_staking_configs(
			origin: OriginFor<T>,
			min_staker_bond: ConfigOp<BalanceOf<T>>,
		) -> DispatchResult {
			ensure_root(origin)?;
