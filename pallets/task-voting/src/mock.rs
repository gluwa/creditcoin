use core::fmt;

use crate::{self as pallet_task_voting, VotingPower};
use frame_support::{parameter_types, traits::ConstU32, BoundedBTreeSet};
use frame_system as system;
use sp_core::H256;
use sp_runtime::{
	testing::Header,
	traits::{BlakeTwo256, IdentityLookup},
	Saturating,
};

type UncheckedExtrinsic = frame_system::mocking::MockUncheckedExtrinsic<Test>;
type Block = frame_system::mocking::MockBlock<Test>;
pub type BlockNumber = u64;

// Configure a mock runtime to test the pallet.
frame_support::construct_runtime!(
	pub enum Test where
		Block = Block,
		NodeBlock = Block,
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		System: frame_system,
		TaskVoting: pallet_task_voting,
	}
);

parameter_types! {
	pub const BlockHashCount: u64 = 250;
	pub const SS58Prefix: u8 = 42;
}

impl system::Config for Test {
	type BaseCallFilter = frame_support::traits::Everything;
	type BlockWeights = ();
	type BlockLength = ();
	type DbWeight = ();
	type RuntimeOrigin = RuntimeOrigin;
	type RuntimeCall = RuntimeCall;
	type Index = u64;
	type BlockNumber = BlockNumber;
	type Hash = H256;
	type Hashing = BlakeTwo256;
	type AccountId = u64;
	type Lookup = IdentityLookup<Self::AccountId>;
	type Header = Header;
	type RuntimeEvent = RuntimeEvent;
	type BlockHashCount = BlockHashCount;
	type Version = ();
	type PalletInfo = PalletInfo;
	type AccountData = pallet_balances::AccountData<Balance>;
	type OnNewAccount = ();
	type OnKilledAccount = ();
	type SystemWeightInfo = ();
	type SS58Prefix = SS58Prefix;
	type OnSetCode = ();
	type MaxConsumers = ConstU32<{ u32::MAX }>;
}

impl crate::Config for Test {
	type RuntimeEvent = RuntimeEvent;

	type TaskId = u64;

	type OutputId = u64;

	type OnVoteConclusion = ();

	type VotingProvider =
		crate::VotingProviderStrategy<crate::UniformVoterPower, crate::AtLeastOneVote>;

	type MaxVoters = ConstU32<10>;

	// type WeightInfo = ();
}

parameter_types! {
	pub const ExistentialDeposit: u128 = 500;
	pub const MaxLocks: u32 = 50;
}

type Balance = u128;

// Build genesis storage according to the mock runtime.
pub fn new_test_ext() -> sp_io::TestExternalities {
	tracing::try_init_simple();
	let t = frame_system::GenesisConfig::default().build_storage::<Test>().unwrap();
	let mut ext = sp_io::TestExternalities::new(t);
	ext.execute_with(|| System::set_block_number(1));
	ext
}

#[allow(dead_code)]
pub fn roll_to(n: BlockNumber, _author_account_id: u64) {
	let mut i = System::block_number();
	while i < n {
		i += 1;
		System::set_block_number(i);
	}
}

// useful aliases + trait impls for use in tests

pub type Pallet = crate::Pallet<Test>;
pub type ActiveVotes = crate::Rounds<Test>;
pub type AccountId = <Test as frame_system::Config>::AccountId;
pub type MaxVoters = <Test as crate::Config>::MaxVoters;
pub type DataId = <Test as crate::Config>::OutputId;
pub type TaskId = <Test as crate::Config>::TaskId;
pub type VoteData = crate::voting::Data<AccountId, MaxVoters>;
pub type Votes = crate::Votes<AccountId, DataId, MaxVoters>;
pub type Voting = <Test as crate::Config>::VotingProvider;

impl PartialEq for VoteData {
	fn eq(&self, other: &Self) -> bool {
		self.total_voting_power.eq(&other.total_voting_power) && self.voters.eq(&other.voters)
	}
}

impl Default for VoteData {
	fn default() -> Self {
		VoteData::new()
	}
}

impl Default for Votes {
	fn default() -> Self {
		Self { votes: Default::default(), vote_total: Default::default() }
	}
}

impl PartialEq for Votes {
	fn eq(&self, other: &Self) -> bool {
		self.vote_total.eq(&other.vote_total) && self.votes.eq(&other.votes)
	}
}

impl fmt::Debug for Votes {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("Votes")
			.field("votes", &self.votes)
			.field("vote_total", &self.vote_total)
			.finish()
	}
}

impl fmt::Debug for VoteData {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("VoteData")
			.field("total_voting_power", &self.total_voting_power)
			.field("voters", &self.voters)
			.finish()
	}
}

impl FromIterator<(AccountId, VotingPower)> for VoteData {
	fn from_iter<T: IntoIterator<Item = (AccountId, VotingPower)>>(iter: T) -> Self {
		let mut total_voting_power = 0;
		let mut voters = BoundedBTreeSet::new();
		for (voter, power) in iter {
			voters.try_insert(voter).unwrap();
			total_voting_power.saturating_accrue(power);
		}
		Self { total_voting_power, voters }
	}
}
