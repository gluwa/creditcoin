...
impl serde::Serialize for VecString {
	...
	where
		S: serde::Serializer,
	{
		serde::Serialize::serialize(core::str::from_utf8(&self.0).unwrap(), serializer)
	}
}
...
impl JsonRpcRequest {
	...
	pub fn send<T: for<'de> serde::Deserialize<'de>>(self, rpc_url: &str) -> Result<T, ()> {
		let rpc_bytes = serde_json::to_vec(&self).map_err(|e| {
		...
		let response = http::Request::post(rpc_url, vec![rpc_bytes])
			...
			.send()
			.map_err(|err| {
				log::error!("Received http error: {:?}", err);
				()
			})?
			.try_wait(timeout)
			.unwrap()
		...
		let rpc_response: JsonRpcResponse<T> = serde_json::from_slice(&body).unwrap();
...
impl<T> JsonRpcResponse<T> {
	pub fn result(self) -> Result<T, JsonRpcError> {
		...
		if let Some(result) = self.result {
			...
			unreachable!("neither an error nor result found");
		}
	}
}
...
pub fn get_eth_transaction(tx_id: &ExternalTxId, rpc_url: &str) -> Result<EthTransaction, ()> {
	let rpc_req = JsonRpcRequest::new(
		...
		vec![serde_json::Value::String(
			alloc::string::String::from_utf8(tx_id.clone().into()).map_err(|_| ())?,
		)],
	);
