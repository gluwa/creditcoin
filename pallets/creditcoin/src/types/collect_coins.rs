use crate::types::{
	AddressId, Blockchain, ExternalAddress, ExternalTxId, GATEContract, GCreContract, SystemConfig,
};
...
#[derive(Clone, Encode, Decode, Eq, PartialEq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
pub struct BurnGATE<Hash, Balance> {
	pub to: AddressId<Hash>,
	pub amount: Balance,
	pub tx_id: ExternalTxId,
}
...
#[derive(Clone, Encode, Decode, Eq, PartialEq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
pub struct UnverifiedBurnGATE {
	pub to: ExternalAddress,
	pub tx_id: ExternalTxId,
	pub contract: GATEContract,
}

impl UnverifiedBurnGATE {
	pub fn into_output<T>(self, amount: T::Balance) -> BurnGATE<T::Hash, T::Balance>
	where
		T: Config,
	{
		let Self { to, tx_id, contract: GATEContract { chain, .. } } = self;
		let to = crate::AddressId::new::<T>(&chain, to.as_slice());
		BurnGATE { amount, to, tx_id }
	}
}

#[derive(Clone, Encode, Decode, Eq, PartialEq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
pub struct BurnGATEId<Hash>(Hash);
...
impl<H> From<H> for BurnGATEId<H> {
	fn from(hash: H) -> Self {
		Self(hash)
	}
}

impl<T: Config> OffchainVerification<T> for UnverifiedBurnGATE {
	type Output = T::Balance;
...
impl<T: Config> TaskV2<T> for UnverifiedBurnGATE
where
	UnverifiedBurnGATE: OffchainVerification<T>,
	<UnverifiedBurnGATE as OffchainVerification<T>>::Output: Into<T::Balance>,
{
	type Call = crate::pallet::Call<T>;
	type EvaluationError = VerificationFailureCause;
	type SchedulerError = SchedulerError;
	fn to_id(&self) -> T::Hash {
		BurnGATEId::inner_hash::<T::Hashing>(&self.contract.chain, self.tx_id.as_slice())
	}
	...
	fn is_persisted(id: &T::Hash) -> bool {
		let id = BurnGATEId::from(*id);
		crate::pallet::BurnedGATE::<T>::contains_key(id)
	}
}
