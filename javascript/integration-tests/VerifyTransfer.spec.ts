...
describe('VerifyTransfer', (): void => {
    ...
    beforeEach(async () => {
        ...
        authority = keyring.addFromUri(testUtils.AUTHORITY_SURI);
    ...
    it('fee is min 0.01 CTC', async (): Promise<void> => {
        const ccTransfer = testUtils.createCreditcoinTransfer(api, transferEvent.transfer);

        return new Promise((resolve, reject): void => {
            const unsubscribe = api.tx.creditcoin
                .verifyTransfer(ccTransfer)
