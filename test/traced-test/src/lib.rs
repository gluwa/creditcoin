...
impl TestLogs {
	...
	pub fn init_with_config(config: TestTraceConfig) -> Self {
		...
		let subscriber = make_subscriber(writer, &config.filter.unwrap_or_else(|| "trace".to_string()));
		let guard = tracing_core::dispatcher::set_default(&subscriber);
		Self { buf, _guard: guard }
	}

	pub fn init_with_filter(filter: &str) -> Self {
		Self::init_with_config(TestTraceConfig { filter: Some(filter.into()), ..Default::default() })
	}
