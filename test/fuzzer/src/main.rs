...
fn main() {
	...
	let genesis_storage: Storage = {
		use creditcoin_node_runtime::{BalancesConfig, GenesisConfig, GrandpaConfig, SudoConfig};
		use pallet_babe::AuthorityId as BabeId;
		use pallet_grandpa::AuthorityId as GrandpaId;
		use sp_runtime::app_crypto::ByteArray;
		use sp_runtime::BuildStorage;

		let initial_authorities: Vec<(BabeId, GrandpaId)> =
			vec![(BabeId::from_slice(&[0; 32]).unwrap(), GrandpaId::from_slice(&[0; 32]).unwrap())];
	...
	ziggy::fuzz!(|data: &[u8]| {
		let iteratable = Data { data, pointer: 0, size: 0 };
