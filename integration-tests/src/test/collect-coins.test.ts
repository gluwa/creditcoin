...
describe('CollectCoins', (): void => {
    ...
    describe('request', (): void => {
        ...
        it('should fail when txHash not found', async (): Promise<void> => {
            const {
                extrinsics: { requestCollectCoins },
            } = ccApi;

            await expect(requestCollectCoins(deployerRegAddr.item.externalAddress, collector, '0x')).rejects.toThrow(
                'creditcoin.TransactionNotFound',
            );
        }, 900_000);
    });
