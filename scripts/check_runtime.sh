...
echo "make sure we have all tags (including those from the release branch)"
git fetch --depth="${GIT_DEPTH:-100}" origin $RELEASE_BRANCH
...
git fetch --depth="${GIT_DEPTH:-100}" origin $MAIN_BRANCH
...
if ! has_runtime_changes "origin/${MAIN_BRANCH}" "${GITHUB_SHA}"; then
  ...
  case "$((SUBSTRATE_REFS_CHANGED))" in
    ...
    (2)
      ...
      ;;
    (*)
      error "check unsupported: more than one commit targeted in repo ${SUBSTRATE_REPO_CARGO}"
      exit 1
  esac
